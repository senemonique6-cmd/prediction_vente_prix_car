...
MODEL_PATH = BASE_DIR / "rf_regression_model.joblib"
SCALER_PATH = BASE_DIR / "scaler_regression_car.joblib"
...
try:
    ...
    st.error(f"Fichier introuvable : {e.filename}. Placez les fichiers .joblib à côté de app.py.")
    st.stop()

if not isinstance(scaler, Pipeline):
    st.error(
        "Le scaler chargé n'est pas un Pipeline RobustScaler + StandardScaler. "
        "Dans le notebook, les données ont été normalisées avec RobustScaler puis StandardScaler, "
        "mais seul le StandardScaler a été sauvegardé. Sauvegardez le pipeline complet "
        "(voir les instructions) puis relancez l'application."
    )
    st.stop()
